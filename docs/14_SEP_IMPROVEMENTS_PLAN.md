...
- **Status: DONE 2026-09-16, accepted as an opt-in variant.** Converts GDN + attention
  projections only (`scripts/build_fp8_hybrid_snapshot.py`, 156 tensors, 13 GiB sibling
  snapshot; `--quantization modelopt_mixed --fp8-gemm-backend triton`). Decode +13% code,
  +21% ES prose, +12% agentic, +14% streams c=1; boot 466 s; weights 78.85 GB; GSM8K
  196/200 vs 195/200. Costs: prefill TTFT +2.7% at 128k, mixed p95 +3%, arith probe
  −2.5 pts. **`lm_head` is NOT converted** (would break U6's 64k draft token map), and
  hyper-connections cannot be (K=320, and 0.15x at decode). DeepGEMM crashes on SM121.
  See RESEARCH_LOG "Sep 14 plan — D2".
- **Evidence:**
